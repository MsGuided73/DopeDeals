...
import {
  ...
  PRODUCT_CATEGORIES,
  PRODUCT_BRANDS,
} from '../../../lib/product-categorization';
...
const PRODUCT_COLUMNS =
  'id, name, brand_name, price, stock_quantity, featured, tags, materials, image_url';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const products = (rawProducts || []).filter((product) => hasValidImage(product.image_url));
    ...
    if (testType === 'all' || testType === 'brands') {
      ...
      const brandSamples: Record<string, string[]> = {};
    ...
    if (testType === 'all' || testType === 'prices') {
      ...
      const priceSamples: Record<string, Array<{ name: string; price: number }>> = {};
      ...
      results.tests.prices = {
        ...
        minPrice: Math.min(...products.map((p) => parseFloat(p.price) || 0)),
        maxPrice: Math.max(...products.map((p) => parseFloat(p.price) || 0)),
      };
    }
    ...
    if (testType === 'all' || testType === 'stock') {
      ...
      results.tests.stock = {
        ...
        samples: {
          inStock: inStock.slice(0, 3).map((p) => ({ name: p.name, stock: p.stock_quantity })),
          outOfStock: outOfStock.slice(0, 3).map((p) => ({ name: p.name, stock: p.stock_quantity })),
        },
      };
    }
    ...
    if (testType === 'all' || testType === 'featured') {
      ...
      results.tests.featured = {
        ...
        total: products.length,
        samples: {
          featured: featured.slice(0, 3).map((p) => p.name),
          notFeatured: notFeatured.slice(0, 3).map((p) => p.name),
        },
      };
    }
    ...
    if (testType === 'all' || testType === 'tags') {
      ...
      results.tests.tags = {
        ...
        topTags,
        productsWithTags: products.filter((p) => Array.isArray(p.tags) && p.tags.length > 0).length,
      };
    }
    ...
    if (testType === 'all' || testType === 'materials') {
      ...
      results.tests.materials = {
        ...
        topMaterials,
        productsWithMaterials: products.filter(
...
export async function POST(request: NextRequest) {
  try {
    ...
    let filteredProducts = (rawProducts || []).filter((product) => hasValidImage(product.image_url));
    ...
    return NextResponse.json({
      originalCount: rawProducts?.length || 0,
