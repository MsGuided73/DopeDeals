...
async function getZohoAccessToken(): Promise<string> {
  ...
  if (expiresAt > now && tokenRow.access_token) {
    console.log('[Zoho Migration] Using existing valid access token');
    return tokenRow.access_token;
  }

  // Token is expired, refresh it
  console.log('[Zoho Migration] Token expired, refreshing...');
...
async function refreshZohoToken(refreshToken: string, dc: string, orgId: string): Promise<string> {
  ...
  if (updateError) {
    console.warn('[Zoho Migration] Failed to update token in database:', updateError.message);
    // Don't throw error here, we still have the token
  } else {
    console.log('[Zoho Migration] Token refreshed and updated in database');
  }
