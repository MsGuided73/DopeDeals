 ...
 * Flower API Route
 *
 * Returns products with category_slug = "flower"
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(`🎯 Flower API: Retrieved ${rawProducts?.length || 0} active flower products with valid images`);
    ...
    return NextResponse.json({
      ...
      message: 'Flower products retrieved successfully'
    });

  } catch (error) {
    console.error('Error in flower API:', error);
    return NextResponse.json(
