...
export interface ThcaPnvProduct {
  ...
  type?: string; // Cartridge / Disposable / Vaporizer
  size?: string; // 0.5g / 1g / 2g
  material?: string;
  materials?: string[];
}

// Pipe-style "type" derivation for vape product names. Run once per product
// at load time so the filter has data even when the catalog lacks an explicit
// `type` column.
const VAPE_TYPES: Array<{ label: string; keywords: string[] }> = [
  { label: 'Cartridge', keywords: ['cartridge', 'cart '] },
  { label: 'Disposable', keywords: ['disposable', 'disposible', 'all-in-one', 'all in one'] },
  { label: 'Vaporizer', keywords: ['vaporizer', 'vape pen', 'dry herb', 'pax', 'puffco'] },
];
const deriveType = (p: any): string | null => {
  ...
  for (const t of VAPE_TYPES) {
    if (t.keywords.some((kw) => haystack.includes(kw))) return t.label;
  }
  // category_slug fallback: cartridges/carts/vapes/disposables drive the type.
...
// Match common gram-weight patterns (".5g", "0.5g", "1g", "2g", "1 gram") in
...
export default function VapesPageContent() {
  ...
  const [activeCategory, setActiveCategory] = useState('all-vapes');
  ...
  useEffect(() => {
    ...
    // Hero-pill category filter — uses the category_slug from the API plus
    // a derived `type` fallback for products that don't carry the slug.
    if (activeCategory !== 'all-vapes') {
      switch (activeCategory) {
        ...
        case 'disposables':
          ...
          break;
        case 'vaporizers':
          filtered = filtered.filter((p) => p.type === 'Vaporizer');
          break;
      }
    }
  ...
  const loadVapeProducts = async () => {
    try {
      ...
      console.log(`✅ API returned ${totalCount} vape products`);
      ...
      // sidebar inputs show real bounds (e.g. $12 – $89) instead of 0–200.
      ...
      console.error('Error loading vape products:', error);
      setProducts([]);
      setFilteredProducts([]);
    } finally {
      setLoading(false);
  ...
  if (loading) {
    return (
      <LoadingState
        loading={loading}
        onRetry={loadVapeProducts}
        timeout={15000}
      >
        <div>Vapes Page Content</div>
      </LoadingState>
    );
  }
