...
export default function ThcaPageContent() {
  ...
  const fetchThcaProducts = async () => {
    try {
      setLoading(true);
      console.log('Fetching THCA products...');
      const response = await fetch('/api/products/thca?limit=30');
      ...
      if (!response.ok) {
        ...
        console.error('THCA API error:', errorMessage);
        throw new Error(`Failed to fetch THCA products: ${errorMessage}`);
      }

      const data = await response.json();
      console.log('THCA data received:', data);

      if (!data.products) {
        console.warn('No THCA products data received');
        setProducts([]);
        return;
      }
      ...
      console.error('Error fetching THCA products:', err);
