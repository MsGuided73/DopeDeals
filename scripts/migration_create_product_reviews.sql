...
-- ─── Idempotency guard ──────────────────────────────────────────────────────
-- Drop any prior partial state so re-running this migration is always safe.
-- This is intentional and only safe because the reviews feature is net-new and
-- the table cannot yet contain real data. Remove this guard once the feature
-- ships and real reviews exist.
...
CREATE TABLE IF NOT EXISTS public.product_reviews (
  ...
  -- NOTE: order_items.id is TEXT/VARCHAR (not UUID) in this DB despite the
  -- original migration declaring it as UUID — a later schema change altered it.
